...
if  ! [ -x "$(command -v docker)" ]; then
    ...
    echo "Docker command already found, skipping install."
fi


# Check docker post install
if ! [ $(getent group docker) ] || ! id -nG | grep -qw "docker" ; then
    ...
    while true; do
        ...
        case $yn in
            [Yy]* ) 
                ...
                break;;
            [Nn]* ) 
                echo "Skipping docker post install steps. Without it on Linux, this user might have to call docker with sudo everytime."
                break;;
            * ) echo "Please answer y or n";;
        esac
    done
    ...
    echo "Docker Linux post install already done. Skipping step."
fi
...
# Install aica docker
if ! [ -x "$(command -v aica-docker)" ]; then
    while true; do
        ...
        case $yn in
            [Yy]* ) 
                ...
                break;;
            [Nn]* ) 
                echo "Skipping installation of aica-docker."
                break;;
            * ) echo "Please answer y or n";;
        esac
    done
